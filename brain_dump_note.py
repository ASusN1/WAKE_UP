...
class BrainDumpNote:
    def __init__(self):
        ...
        #the text container
        box_brain_dump_note = tk.Frame(self.window, width=450, height=400, bg="white", relief="solid", bd=1)
        ...
        #store data
        store_info_btn = tk.Button(self.window, text ="Save Note", command=lambda: save_brain_dump_note(title_text_bd, self.brain_dump_notes, note_type=self.note_type))
        ...
        finish_btn = tk.Button(self.window, text="Finish", command=lambda: (save_brain_dump_note(title_text_bd, self.brain_dump_notes, note_type=self.note_type), finish_brain_dump_note(self.window)))
